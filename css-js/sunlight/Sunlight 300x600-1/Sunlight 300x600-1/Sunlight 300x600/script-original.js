$(document).ready(function () {
    ...
    let options = [
      ...
      for (let i = 0; i < options.length; i++) {
        ...
        for (let j = 0; j < options[i].length; j++) {
          questionOptionsHTML += `
              <li class="option" data-value="${options[i][j]}">${options[i][j]}</li>
          `;
        }
            ...
            if(Fake[i].options != ""){
                ...
                options.forEach((option, i) => {
                    option.addEventListener('click', () => {
                        ...
                        msgId++;
                        console.log(msgId);
                        setTimeout(Fake[i].trailers.forEach(displayTrailers), 1000);
                        return;
                    });
                });
            }else{
 ...
 trailing = 
    {
        one: [
            'Sunlihght comes in 2 in 1 form and is designed to keep your hands safe',
            'Sunlight 2 in 1 Auto is the number one made for machine detergent in Kenya',
            'Sunlight 2 in 1 Auto is ideal for washing machines because it is gentler and requires less water every cycle than normal washing powder',
        ],
        two: [
    ...
    // async function displayTrailers(trail){
    //     await delay(1000);
    //     updateScrollbar();
